...
class bilibili_(QWidget):
    ...
    def initUI(self):
        ...
        self.down_address=QLineEdit("https://www.bilibili.com/video/BV18x411U7mM?from=search&seid=9315818682331035811")
    ...
    def startDownload(self):
        ...
        url_api = str(self.down_address.text())
        #print url_api
        



        headerss={"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36","Referer":"https://www.bilibili.com/video/"}
        
        video_address = url_api
        #print video_address

        #self.down_address.setText(video_address)

        downpos=str(self.save_address.text()).decode('utf-8')


        title=str(self.file_name.currentText()).decode("utf-8")
        
        rstr = r"[\/\\\:\*\?\"\<\>\|♪]"  
        new_title = re.sub(rstr,"", title)

        #print new_title.decode('utf-8')
        if "1" not in new_title:
            new_name=downpos+"/" + new_title.decode('utf-8',"ignore")+".flv"
        else:
            
            new_title=new_title.replace("1",".wav")
            new_name=downpos+"/" + new_title.decode('utf-8',"ignore")
    ...
    def setData(self):#创建选择菜单
        global down_list
        if down_list!={}:
            cid=down_list[str(self.file_name.currentText()).decode("utf-8")]

            regt= re.compile(r'https://.*?&avid=(\d+)&.*?')
            aid=re.findall(regt,str(self.down_address.text()))[0]
            
            url_api = 'https://api.bilibili.com/x/player/playurl?cid={}&avid={}&qn={}'.format(cid, aid, self.quality)
            #print url_api

            self.down_address.setText(url_api)
    ...
    def resolve(self):
        QMessageBox.information(self,u"提示", u"只有真实地址下载功能！")
